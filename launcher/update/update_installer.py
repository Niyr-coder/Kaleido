    ...
    def prepare_installation(
        ...
        try:
            with open(batch_path, "w", encoding="utf-8") as batch:
                ...
                batch.write('set "WAITPID=%~1"\n')
                batch.write(f'set "EXENAME={exe_name}"\n')
                batch.write('echo [%date% %time%] Update start (waiting for pid %WAITPID%) > "%LOG%"\n')
                # Kaleido: wait for the launching process AND any other instance of the app to exit
                # before touching the install dir. The old script slept 3 s and then robocopy hit a
                # still-running exe with 30 s retries, and a second update could overlap the first.
                batch.write("set /a TRIES=0\n")
                batch.write(":waitpid\n")
                batch.write('if "%WAITPID%"=="" goto :waitexe\n')
                batch.write('tasklist /FI "PID eq %WAITPID%" 2>nul | find "%WAITPID%" >nul\n')
                batch.write("if errorlevel 1 goto :waitexe\n")
                batch.write("set /a TRIES+=1\n")
                batch.write("if %TRIES% GEQ 90 goto :waitexe\n")
                batch.write("ping 127.0.0.1 -n 2 >nul\n")
                batch.write("goto :waitpid\n")
                batch.write(":waitexe\n")
                batch.write("set /a TRIES=0\n")
                batch.write(":waitexeloop\n")
                batch.write('tasklist /FI "IMAGENAME eq %EXENAME%" 2>nul | find /I "%EXENAME%" >nul\n')
                batch.write("if errorlevel 1 goto :ready\n")
                batch.write("set /a TRIES+=1\n")
                batch.write("if %TRIES% GEQ 60 goto :ready\n")
                batch.write("ping 127.0.0.1 -n 2 >nul\n")
                batch.write("goto :waitexeloop\n")
                batch.write(":ready\n")
                batch.write('echo [%date% %time%] Application closed, copying files >> "%LOG%"\n')
                batch.write('if not exist "%SOURCE%\\%EXENAME%" (\n')
                batch.write('    echo [%date% %time%] Staging is missing %EXENAME% - aborting >> "%LOG%"\n')
                batch.write("    goto :robofail\n")
                batch.write(")\n")
                batch.write('robocopy "%SOURCE%" "%DEST%" /E /R:5 /W:2 /NFL /NDL /NJH /NJS /XD __pycache__ "Pengu Loader\\plugins" /XF config.ini cslol-dll.dll hashes.game.txt >> "%LOG%" 2>&1\n')
                ...
                batch.write('        robocopy "%%D" "%DEST%\\Pengu Loader\\plugins\\%%~nxD" /MIR /R:5 /W:2 /NFL /NDL /NJH /NJS /XD __pycache__ >> "%LOG%" 2>&1\n')
                batch.write('    )\n')
                batch.write(')\n')
